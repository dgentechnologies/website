...
function EditorContent() {
  ...
  const handleGenerateWithAI = useCallback(async (title: string, currentAuthor: string) => {
    if (!title || !currentAuthor) {
      toast({
        variant: 'destructive',
    ...
    try {
      const result = await generateBlogPost({ 
        topic: title, 
        author: currentAuthor as BlogAuthor
      });
      setGeneratedContent({
        title: result.title,
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-64px)] p-4 md:p-6">
      ...
      <div className="flex-1 min-h-0">
        <BlogEditor
          initialTitle={generatedContent.title || topic}
          ...
          initialAuthor={generatedContent.author || author}
